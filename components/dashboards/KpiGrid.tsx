...
const KpiGrid: React.FC<{ kpis: Kpi[] }> = ({ kpis }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    {kpis.map((k) => (
      <div key={k.label} className="gi-card p-6">
        <div className="text-sm font-medium gi-muted2">{k.label}</div>
        <div className="mt-1 text-2xl font-bold gi-kpi-value">{formatValue(k)}</div>
        {k.help && <div className="mt-2 text-xs gi-muted2">{k.help}</div>}
      </div>
    ))}
  </div>
);
